...
与式\eqref{eq:logistic}相比,
式\eqref{eq:lv-comp:part}仅添加$\alpha_{i,j} \frac{x_j}{k_i}$一项(简称“交互项”),
...
式\eqref{eq:lv-comp}将 Logistic 方程\eqref{eq:logistic}中$\frac{x_i}{k_i}$一项并入求和,
既是为了保持方程形式简洁,也为引入
\strong{\ref{sec:models:lv-comp:inter-mat}交互矩阵}提供方便.
...
观察式\eqref{eq:lv-pred:part:1}与式\eqref{eq:lv-pred:part:2}的形式,
...
\end{equation}
由于式\eqref{eq:lv-pred}的$\beta_{i,j}$与\ref{sec:models:lv-comp:desc}中的$\alpha_{i,j}$一样,
...
否则等式\eqref{eq:lv-pred}右侧将出现平方增长项$\beta_{i,i} x_i^2$,导致该种群无限增长.
...
由三种基本生态学模型导出,取特殊参数时可退化为三种基本模型.
它是研究复杂生态系统演化的基础,在\ref{sec:example}实例分析中发挥重要作用.
...
或更为紧凑的矩阵形式
\begin{equation}
    \boxed{\boxed{\frac{\mathrm{d} \mathbf{x}}{\mathrm{d} t} = \mathbf{x} \circ (\mathbf{I} - \boldsymbol{\alpha} \mathbf{x}) \circ (\mathbf{r} + \boldsymbol{\beta} \mathbf{x}) + \mathbf{f}.}}
\end{equation}
...
\begin{figure}[h]
    ...
    \label{fig:general}
\end{figure}
图\ref{fig:general}展示了
$\mathbf{x_0} = \langle 60, 20 \rangle$, 
$\mathbf{r} = \langle 0.1, -0.1 \rangle$, 
$\mathbf{k} = \langle 100, 150 \rangle$, 
$\boldsymbol{\alpha} = \left[ \begin{smallmatrix}
    1 & 2 \\
    3 & 1
