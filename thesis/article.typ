...
#show: elegant-paper.with(
  ...
  title: (
    ...
    abstract: "
    生态学在农业生产中常被用于预测农作物产量。本文使用计算机分析方法，就不同生态因素对叶菜类农作物产量影响进行建模，得到了综合种内、种间竞争、天敌影响、土壤肥力等生态因素综合作用下的叶菜类农作物种群数量模型，最终得以精准预测、精准干预。
    ",
    keywords: ()
  ),
)
...
=== 简述
<简述>
皮埃尔-弗朗索瓦·韦吕勒（P.F.Verhulst, 1804-1849）于 1838
年提出逻辑斯蒂方程 @primer_eco。
...
$ (dif N) / (dif t) = r N (1 - N / K) $
此为一阶常微分方程。利用微积分知识，可得到如下通解:
$ N_t = K / (1 + (K / N_0 - 1) e^(- r t)) $
...
每个种群均按照逻辑斯蒂方式增长, 有自己的内禀增长率 ($r_1$ 或 $r_2$)
和环境容纳量 ($K_1$ 或 $K_2$)。 与逻辑斯蒂模型一样,
种内竞争抑制种群增长:
$ frac(upright(d) N_1, upright(d) t) = r_1 N_1 (frac(K_1 - N_1 - alpha N_2, K_1)) $
$ frac(upright(d) N_2, upright(d) t) = r_2 N_2 (frac(K_2 - N_2 - beta N_1, K_2)) $

#figure(image("../figures/lotka-volterra.pdf"), caption: [
  Lotka-Volterra 竞争模型
])
...
$ frac(upright(d) V, upright(d) t) = r_m (1 - V / K) - c_1 H (frac(1 - e^(- d_1) V, V)) $
...
食肉动物而变得复杂起来:
$ frac(upright(d) H, upright(d) t) = a_2 + c_2 (1 - e^(d_2) V) - f P (frac(1 - e^(- d_3) H, H)) $
对于食肉动物，我们则采用一个新方程加以描述:
$ frac(upright(d) P, upright(d) t) = - a_3 + c_3 (1 - e^(d_4) H) $
...
模型的绝对准确。复杂计算机模型的校准需
要明确处理结构的误差——意识到模型只是
对现实的近似，结果本身包含一定误差，这 对决策者而言也是非常重要的信息。
...
定决策。这或许正是"对种群动态的数学手
段模式化分析"对农作物生产场景的最根本
贡献。当然了，我们也要看到研究的局限性。 从空间的角度看，如在 LV
竞争模型中，如 果我们拥有一个更加广阔的充足空间，就需
要考虑到种群的空间分布对于其数量变化的
影响。例如大型食肉动物对小型动物的威慑
作用。从时间的角度看，因其还不具备引入
长期复杂现实因素的能力，可能在更加广阔
